import { ensureArray, ensureBoolean, ensureObject, isString } from "https://deno.land/x/unknownutil@v1.1.0/mod.ts";
...
export function ensureAugendConfigConstant(x: unknown): asserts x is AugendConfigConstant {
  ensureObject(x);
  ensureArray(x.elements, isString);
  if (x.elements.length <= 1) {
    throw new Error("The number of elements must be greater than or equal to 2.");
  }
...
export function augendConstant(conf: AugendConfigConstant) : Augend {
  ...
  const cyclic = (conf.cyclic === undefined) ? true : conf.cyclic;
  const word = (conf.word === undefined) ? true : conf.word;
